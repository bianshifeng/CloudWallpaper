...
ButtonStyle {
    ...
    background: View {
        ...
        Ink {
            ...
            focusWidth: parent.width - 30 * g_dp
    ...
    label: Item {
        implicitHeight: Math.max(36 * g_dp, label.height + 16 * g_dp)
        implicitWidth: context == "dialog"
                ? Math.max(64 * g_dp, label.width + 16 * g_dp)
                : context == "snackbar" ? label.width + 16 * g_dp
                                        : Math.max(88 * g_dp, label.width + 32 * g_dp)
