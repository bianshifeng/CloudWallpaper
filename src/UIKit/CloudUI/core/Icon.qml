...
Item {
    ...
    property string imageResPath:"qrc:/qml/Images/icons/"

    property color color: Theme.light.iconColor
    property real size: 24 * g_dp
    ...
    Image {
        ...
        source: {
            if (icon.source.indexOf("icon://awesome/") == 0) {
                ...
                if (name)  //qrc:/qml/Images/icons/navigation_close.svg
                    return imageResPath + name + '.svg'
                else
                    return ""
            } else {
                return icon.source
